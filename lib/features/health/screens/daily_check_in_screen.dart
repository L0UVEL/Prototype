...
class _DailyCheckInScreenState extends State<DailyCheckInScreen> {
  ...
  Future<void> _submitCheckIn() async {
    ...
    if (user != null) {
      ...
      if (_mood == 'Bad' || _mood == 'Terrible' || _selectedSymptoms.isNotEmpty) {
        status = 'At Risk';
      }

      final symptomsString = _selectedSymptoms.join(', ');
      final moodString = 'Mood: $_mood';
      final notesString = _notesController.text.isNotEmpty ? 'Notes: ${_notesController.text}' : '';
      
      final compositeSymptoms = [moodString, symptomsString, notesString]
          .where((s) => s.isNotEmpty)
          .join('. ');

      final log = HealthUpdate(
        updateId: const Uuid().v4(),
        studentId: user.studentId,
        userId: user.id,
        checkinDate: DateTime.now(),
        symptoms: compositeSymptoms,
        status: status,
      );
